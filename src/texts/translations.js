export default {
  en: {
    //about us
    help: "Let us help You!",
    church: "Church events:",
    thursday: "Thursday",
    sunday: "Sunday",
    description:
      "If you are searching for your place, if you are searching for who you are, if you are looking for a place where you can be yourself, if you are looking for someone to call a friend, if you are searching for God… THIS IS THE PLACE FOR YOU! If you are studying in Oradea and want to be part of a welcoming community, we invite you to Váradifi. We meet every Wednesday at 7 PM at **5 Berthelot Street**, and we would love for you to join our gatherings. Give us a chance—you won’t regret it!",
    shortInfo:
      "A Christian youth group in Oradea that meets weekly to talk, worship, and learn from the Bible. We invite you to join us every Wednesday at 7 PM!",
    //header
    home: "Home",
    eventS: "Events",
    shirts: "Shirts",
    aboutUs: "About",
    placeOrder: "Order",
    //home
    bigTitle: "This is your place!",
    groupName: "Oradea Baptist Youth Group",
    placeholderName: "Enter your name",
    dateMonth: "Month: {month}",
  },
  hu: {
    //Rolunk
    help: "Szólj ha segíthetünk!",
    church: "Gyülekezeti alkalmaink:",
    thursday: "Csütörtök",
    sunday: "Vasárnap",
    description:
      "  Ha keresed a helyedet, ha keresed, hogy ki vagy, ha keresel egy helyet, ahol önmagad lehetsz, ha keresel valakit, akit barátnak nevezz, ha keresed Istent… ITT A HELYED! Ha Nagyváradon tanulsz és szeretnél egy befogadó közösség része lenni, akkor várunk a Váradifibe. Minden héten szerdán 7 órától találkozunk a Berthelot utca 5. szám alatt, és szeretnénk, ha te is részese lennél alkalmainknak. Adj egy lehetőséget nekünk, nem bánod meg! ",
    shortInfo:
      "Nagyváradi keresztény ifjúsági csoport, akik heti szinten találkozunk, hogy beszélgessünk, dicsőitsünk és tanuljunk a Biblából. Várunk téged is minden szerdán 7től!",
    //fejlec
    home: "Kezdőlap",
    eventS: "Események",
    shirts: "Pólók",
    aboutUs: "Rólunk",
    placeOrder: "Rendelj",
    //kezdolap
    bigTitle: "Itt a helyed!",
    groupName: "Nagyváradi Baptista Ifjúság",
    placeholderName: "Add meg a neved",
    dateMonth: "Hónap: {month}",
  },
};
